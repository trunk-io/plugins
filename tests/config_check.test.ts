 ...
 */
const autoEnabledLinters = new Set([
  "actionlint",
  "black",
  "buildifier",
  "cfnlint",
...
// Additional type definitions are needed in order to provide custom matcher.
// trunk-ignore(eslint/@typescript-eslint/no-unused-vars)
type ConfigMatcher<Params extends unknown[]> = (
...
declare global {
  // trunk-ignore(eslint/@typescript-eslint/no-namespace): Type declarations must be explicitly overriden.
  namespace jest {
    // trunk-ignore(eslint/@typescript-eslint/no-unused-vars)
    interface Matchers<R, T> {
      toBeAutoEnabled(linterName: string): T;
    }
    interface ExpectExtendMap {
      toBeAutoEnabled: ConfigMatcher<[linterName: string]>;
    }
  }
}

// Custom matcher allows for additional information about failing linter to be provided at test-time.
expect.extend({
  toBeAutoEnabled: (actual, linterName: string) => {
    const expected = autoEnabledLinters.has(linterName);
    const pass = actual === expected;
    return {
      pass,
      message: pass
        ? () => `expected ${linterName} ${expected ? "not " : ""}to be auto-enabled`
        : () => `expected ${linterName} ${expected ? "" : "not "}to be auto-enabled`,
    };
  },
});
...
describe("Global config health check", () => {
  ...
  it("validate auto-enabled linters", async () => {
    ...
    compositeYaml.lint.definitions.forEach((linterDefinition: any) => {
      const name = linterDefinition.name;
      ...
      const recommendedConfigExists =
        ...
        linterDefinition.direct_configs.reduce((accumulator: boolean, directConfigFile: any) => {
          const expectedConfigPath = path.resolve(REPO_ROOT, localRelativePath, directConfigFile);

          return accumulator || fs.existsSync(expectedConfigPath);
        }, false);
